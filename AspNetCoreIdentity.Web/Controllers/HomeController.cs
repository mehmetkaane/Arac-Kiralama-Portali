...
namespace AspNetCoreIdentity.Web.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            return RedirectToAction(nameof(HomeController.AracArama), new { id = id });
        }
        ...
        public async Task<IActionResult> AracArama(string id)
        {
            List<Arac> araclar;
            if (String.IsNullOrEmpty(id))
                araclar = await _context.Arac.Where(x => x.KiradaMi == false).ToListAsync();
            else
                araclar = await _context.Arac.Where(x => (x.Marka.Contains(id) || x.Model.Contains(id) || x.Kategori.Contains(id) || x.Vites.Contains(id)) && x.KiradaMi == false).ToListAsync();
            return View(araclar);

        }


        public async Task<IActionResult> AracSecim(int id)
        {
            ...
            return RedirectToAction("AracKirala", "Member", new { id = id });
